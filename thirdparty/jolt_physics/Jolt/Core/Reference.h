...
/// Derive your class from RefTarget and you can reference it by using Ref<classname> or
/// RefConst<classname>
///
...
public:
	...
	/// Mark this class as embedded, this means the type can be used in a compound or constructed on
	/// the stack. The Release function will never destruct the object, it is assumed the destructor
	/// will be called by whoever allocated the object and at that point in time it is checked that
	/// no references are left to the structure.
	...
	inline void Release() const
	{
		...
		if (old_value == 1) {
			// ... so that we can use acquire to ensure that we see any updates from other threads
			// that released a ref before deleting the object
			atomic_thread_fence(memory_order_acquire);
			delete static_cast<const T*>(this);
		}
#else
		// But under TSAN, we cannot use atomic_thread_fence, so we use an acq_rel operation
...
protected:
	static constexpr uint32 cEmbedded = 0x0ebedded; ///< A large value that gets added to the
													///< refcount to mark the object as embedded
...
public:
	...
	uint64 GetHash() const { return Hash<T*>{}(mPtr); }
...
public:
	...
	/// Get hash for this object
	uint64 GetHash() const { return Hash<const T*>{}(mPtr); }
...
template <class T> struct hash<JPH::Ref<T>>
{
	size_t operator()(const JPH::Ref<T>& inRHS) const { return size_t(inRHS.GetHash()); }
};

/// Declare std::hash for RefConst
template <class T> struct hash<JPH::RefConst<T>>
{
	size_t operator()(const JPH::RefConst<T>& inRHS) const { return size_t(inRHS.GetHash()); }
};
} // namespace std
