...
def write_bytes(file, source):
    with open(source, mode="rb") as binary:
        buffer = binary.read()
...
def run_make_ltc_lut(target, source):
...
def generate_version(
    argcount, const=False, returns=False, required=False, compat=False
):
    s = proto
    if compat:
    ...
    if argcount:
        ...
        s = s.replace(
            "$CALLPTRARGPASS", "reinterpret_cast<GDExtensionConstTypePtr *>(argptrs)"
        )
    else:
...
def run_make_virtuals(target, source):
    max_versions = 12
...
match n:
    ...
    case 1:
        run_make_ltc_lut(
            ...
            [
                "servers/rendering/storage/make_ltc_lut.py",
                "servers/rendering/storage/ltc/ltc_lut1.dds",
                "servers/rendering/storage/ltc/ltc_lut2.dds",
            ],
        )
    case _:
