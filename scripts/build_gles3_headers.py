...
def include_file_in_gles3_header(
    filename: str, header_data: GLES3HeaderStruct, depth: int
):
    with open(filename, "r", encoding="utf-8") as fs:
        ...
        while line:
            ...
            if (
                line.strip().find("out ") == 0 or line.strip().find("flat ") == 0
            ) and line.find("tfb:") != -1:
...
""")
        ...
        if header_data.uniforms:
            uniforms = ",\n\t\t".join(
                uniform.upper() for uniform in header_data.uniforms
            )
            file.write(f"""\
	enum Uniforms {{
		{uniforms},
...
""")
        ...
        if header_data.uniforms:
            uniforms = ",\n\t\t\t".join(
                f'"{uniform}"' for uniform in header_data.uniforms
            )
            file.write(f"""\
...
""")
        ...
        if header_data.variant_defines:
            ...
            variant_defines = ",\n\t\t\t".join(
                f'"{define}"' for define in header_data.variant_defines
            )
            file.write(f"""\
...
""")
        ...
        if header_data.texunits:
            texunits = ",\n\t\t\t".join(
                f'{{ "{name}", {texunit} }}' for name, texunit in header_data.texunits
            )
            file.write(f"""\
...
""")
        ...
        if header_data.ubos:
            ubos = ",\n\t\t\t".join(
                f'{{ "{name}", {ubo} }}' for name, ubo in header_data.ubos
            )
            file.write(f"""\
...
match run:
    ...
    case 2:
        build_gles3_headers(
            ["drivers/gles3/shaders/feed.glsl.gen.h"], ["drivers/gles3/shaders/feed.glsl"]
        )
    case 3:
        build_gles3_headers(
    ...
    case 5:
        build_gles3_headers(
            ["drivers/gles3/shaders/canvas.glsl.gen.h"], ["drivers/gles3/shaders/canvas.glsl"]
        )
    case 6:
        build_gles3_headers(
    ...
    case 8:
        build_gles3_headers(
            ["drivers/gles3/shaders/scene.glsl.gen.h"], ["drivers/gles3/shaders/scene.glsl"]
        )
    case 9:
        build_gles3_headers(
            ["drivers/gles3/shaders/sky.glsl.gen.h"], ["drivers/gles3/shaders/sky.glsl"]
        )
    case 10:
        build_gles3_headers(
            ["drivers/gles3/shaders/tex_blit.glsl.gen.h"],
